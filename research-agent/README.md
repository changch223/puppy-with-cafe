...
| hours_mon 〜 hours_sun | 曜日別 `9:00-18:00`（複数帯は`,`区切り）/ `定休` / 空=不明。**分かる場合のみ**（これがあると🟢営業中バッジが出る）。閉店は `24:00` まで可、開店>=閉店（例 `18:00-2:00`）は深夜跨ぎ営業として構造化できる（2026-08-08改訂） |
...
**よくある検証エラー**: conditional なのに claimed_condition が空／unverified 以外なのに verified_at が空／同名の店が50m以内に2行（重複→統合）／時刻形式（`9:00-18:00` 形式。2026-08-08改訂: 閉店は `24:00` まで可・開店>=閉店（例 `18:00-2:00`）は深夜跨ぎ営業として入力可。ただし開店=閉店は不可（24時間営業は `00:00-24:00` と明示））。
